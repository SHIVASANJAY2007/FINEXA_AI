  ...
  try {
    ...
    if (!N8N_WEBHOOK_URL) {
      return res.status(503).json({
        error: "N8N_WEBHOOK_URL is not configured. The chatbot strictly requires an active n8n Webhook URL and does not serve static responses."
      });
    }
    ...
    try {
      ...
      return res.status(502).json({ error: `Failed to connect to n8n webhook: ${fetchError.message || fetchError}` });
    } finally {
    ...
    if (!response.ok) {
      return res.status(response.status >= 500 ? response.status : 502).json({
        error: `n8n webhook responded with HTTP status ${response.status}`
      });
    }
    ...
    if (rawText && rawText.trim() !== '') {
      ...
      responseData = "Request processed successfully by n8n.";
    }
