...
const Hero = () => {
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            ...
            mm.add("(min-width: 1024px)", () => {
                ...
                // 2. Reveal each card sequentially
    ...
    return (
        <div
            ref={containerRef}
            className="w-full bg-ivory text-ink relative min-h-screen overflow-hidden flex flex-col justify-center pt-24 sm:pt-28 md:pt-32 pb-16 dot-grid linen-noise"
        >
            {/* Ambient Blooms */}
            ...
            <div ref={contentRef} className="max-w-7xl mx-auto px-4 sm:px-8 md:px-12 w-full z-10 grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center mt-2 sm:mt-4">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 sm:gap-2.5 w-full">
                            {CARDS.map((card, index) => (
                                <div
                                    ...
                                    className={`${card.color} ${card.shape} ${index === 4 ? 'col-span-2 sm:col-span-1' : ''} w-full p-2.5 sm:p-3 flex flex-col justify-between items-center text-center relative group overflow-hidden transition-all duration-300 hover:scale-105 shadow-[0_6px_20px_rgba(58,46,37,0.12)] border border-beige/20 min-h-[96px] sm:min-h-[118px]`}
                                >
                                    <div className="p-1.5 sm:p-2 bg-ivory/15 rounded-xl group-hover:scale-110 transition-transform duration-300 mb-1">
                                        {card.icon}
                                    </div>
                                    <div>
                                        <h3 className="text-[10.5px] sm:text-xs font-extrabold uppercase leading-tight tracking-wider mb-0.5">
                                            {card.title}
                                        </h3>
                ...
                {/* Right Column: Refined Phone Mockup */}
                <div className="lg:col-span-5 flex justify-center lg:justify-end lg:pr-2 xl:pr-4 w-full" style={{ perspective: '1400px' }}>
                    <motion.div
                        ...
                        className="cursor-pointer w-full max-w-[340px] sm:max-w-[365px] lg:max-w-[385px]"
                    >
